export const FETCH_VOUCHERS_REQUEST = "FETCH_VOUCHERS_REQUEST";
export const FETCH_VOUCHERS_SUCCESS = "FETCH_VOUCHERS_SUCCESS";
export const FETCH_VOUCHERS_FAIL = "FETCH_VOUCHERS_FAIL";

export const FETCH_MYVOUCHERS_REQUEST = "FETCH_MYVOUCHERS_REQUEST";
export const FETCH_MYVOUCHERS_SUCCESS = "FETCH_MYVOUCHERS_SUCCESS";
export const FETCH_MYVOUCHERS_FAIL = "FETCH_MYVOUCHERS_FAIL";

export const PLACE_ORDER_REQUEST = "PLACE_ORDER_REQUEST";
export const PLACE_ORDER_SUCCESS = "PLACE_ORDER_SUCCESS";
export const PLACE_ORDER_FAIL = "PLACE_ORDER_FAIL";

export const CLEAR_ERRORS = "CLEAR_ERRORS";
export const CLEAR_MESSAGES = "CLEAR_MESSAGES";
